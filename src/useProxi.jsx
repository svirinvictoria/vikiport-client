    ...
    async function updateWeatherPr(){
     ...
     async function updateForexPr(){
        try {
            ...
            return <div>
                <div><img src={usd_img} className="coin-icon"/>
                    USD (USA) = {dollar}</div>
                <div>
                <img src={eur_img} className="coin-icon"/>Euro (EU) = {euro}</div>
                <div>
                <img src={pound_img} className="coin-icon"/>Pound Sterling (GB) = {pound}</div>
                <div>
                <img src={yen_img} className="coin-icon"/>Yen (Japan) = {yen} </div>
                <div> <img src={shekel_img} className="coin-icon"/>
                    Shekel (Israel) = {shekel} </div>
            </div>;
        } catch (error){
            console.log(error);
